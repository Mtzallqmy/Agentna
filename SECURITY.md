...
Agentna is Android-first and has no Agentna application server. Agent orchestration, local tools, approvals, workspace files, execution logs and automations stay on the device. Model prompts/context are sent directly to the provider selected by the user.

- API keys are encrypted at rest using AES-GCM with a key held by Android Keystore.
...
Do not publish exploitable details in a public issue. Use GitHub private vulnerability reporting for this repository when available. Include the affected version, reproduction steps, impact and any suggested mitigation.
